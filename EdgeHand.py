...
class EdgeHand(object):
    ...
    def getPort(self)-> Tuple[Peer, int]:
        if self.peerList:
    ...
    def getRecv(self, peer: Peer, port: int) -> Message:
        s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        s.bind(('', port))
        s.listen(True)
        conn, addr = s.accept()
    ...
    def getBalance4Addr(self, wallet_addr: str = None) -> int:
        with self.chain_lock:
            ...
            if Utils.send_to_peer(message, peer):
                ...
                message = self.getRecv(peer, port)
                if message:
    ...
    def makeTransaction(self, to_addr, value: int = 0, fee: int = 0) -> Transaction:
        ...
        txin = [self.makeTxin(utxo.outpoint, txout) for utxo in utxos_to_spend]
    ...
    def sendTransaction(self, to_addr, value):
        with self.chain_lock:
            txn = self.makeTransaction(to_addr, value)

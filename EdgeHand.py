...
class EdgeHand(object):
    ...
    def _makeTransaction(
        self, txinType, to_addr, value: int = 0, fee: int = 0
    ) -> Transaction:
        utxos_to_spend = set()
        if txinType == 0:
            utxos = list(
                sorted(
                    self.getUTXO4Addr(self.wallet.my_address),
        ...
        if txinType == 1:
            utxos = list(
                sorted(
                    self.getUTXO4Addr(self.getMultiAddress()),
                    key=lambda i: (i.value, i.height),
                )
            )
        ...
        txout.append(
            TxOut(value=change, pk_script=self._make_pk_script(self.wallet.my_address))
        )
        txin = [
            self._makeTxin(txinType, utxo.outpoint, txout) for utxo in utxos_to_spend
        ]
    ...
    def _makeTxin(self, txinType, outpoint: OutPoint, txout) -> TxIn:
        def build_spend_message(to_spend, pk, sequence, txouts):
            ...
            spend_msg = Utils.sha256d(
                Utils.serialize(to_spend)
                + str(sequence)
                + binascii.hexlify(pk).decode()
                + Utils.serialize(txouts)
            ).encode()
        ...
        if txinType == 0:
            ...
            return TxIn(
                to_spend=outpoint,
                signature_script=self._make_signature_script(txinType, signature, pk),
                sequence=sequence,
            )
            ...
            signature = [
                self.wallet.keypairs[i].sign(
                    build_spend_message(outpoint, pk[i], sequence, txout)
                )
            ...
            return TxIn(
                to_spend=outpoint,
                signature_script=self._make_signature_script(
                    txinType, signature, redeem_script
                ),
                sequence=sequence,
            )
        else:
    ...
    def _make_signature_script(self, txin_type, signature, pk):
        # use template
        return scriptBuild.get_signature_script_without_hashtype(
            txin_type, signature, pk
        )
        ...
        with self.chain_lock:
            ...
            with socket.create_connection(peer, timeout=25) as s:
                ...
                if message:
                    ...
                    logger.info(
                        f"[EdgeHand] recv nothing for Balance4Addr from peer {peer}"
                    )
                    return None
    ...
    def getTxStatus(self, txid: str) -> str:
        """
        https://github.com/EdgeIntelligenceChain/EdgenceChain/blob/6f84a10b67a4552c4dbda82ce37451a5c50dcc15/p2p/TCPserver.py#L390
        handleTxStatusReq func in TCPServer.py

        status = 0, tx found in mempool
        status = 1, tx mined in block
        status = 2, tx not found
        """
    ...
    def getMultiAddress(self) -> str:
        with self.chain_lock:
            ...
            verifying_key = [
                self.wallet.keypairs[i].get_verifying_key().to_string()
